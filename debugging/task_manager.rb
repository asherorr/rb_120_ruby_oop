...
#The issue lies in the local variable assigmment `tasks` on line 38.
#When we're reassigning an instance variable within an instance method, it must be preced with `self.`
#Otherwise, Ruby assumes we're referring to the local variable `tasks`, which hasn't been assigned
#a value yet, which makes its value `nil` upon invoking the method.

#We can also change the local variable to be more descriptive, like `high_priority_tasks`
# (I made this change below.)
