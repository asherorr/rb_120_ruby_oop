...
#However, when testing the program, an exception is raised.
...
#Private methods can only be invoked on `self`.
#In the first version of the code, there was an attempt
#to invoke `serial_number` on an object that is not the current instance.
#This can be remedied by making `serial_number` a protected method.