...
#Make the necessary changes to the code so that the various lengths can be properly sorted.
...
#An ArgumentError is occuring because invoking the `sort` method on line 133 is 
#invoking the `<=>` operator on each `Length` object in the array, and the class
#does not define the `<=>` operator.
#In the second version of the code, I defined the `<=>` method.
